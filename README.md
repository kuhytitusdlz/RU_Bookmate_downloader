...
Загрузчик книг и аудиокниг с B o o k s . Y a n d e x (Bookmate) с поддержкой:
- OAuth-авторизации (окно входа), сохранением токена в `token.txt`;
- выбора **качества аудио** из `playlists.json` с **автопонижением** при ошибках 5xx и печатью тела ответа;
- **пакетного режима** (список ссылок как у yt-dlp) и **архива** уже скачанных ID;
- склейки глав аудиокниг с главами и обложкой (FFmpeg).

Каталоги и файлы создаются в `mybooks/...`.
...
- **5xx при загрузке аудиоглав**  
  В логе увидите `Available offline variants: ...`. Скрипт сам понизит качество и повторит попытку.  
  Если все варианты вернули 5xx — будет выведено **тело ответа**, а затем выполнится обычный повтор с бэкоффом.

- **Прокси/сеть**  
  Укажите `--proxy` или переменную `BOOKMATE_PROXY`, при необходимости увеличьте `--throttle` и `--max-retries`.
